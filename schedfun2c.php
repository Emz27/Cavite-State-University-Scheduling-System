...
{  session_start();
	        	...
	        	$start = $j.':'.$v;	
				    ...
				    ///////////////////////////////////////////////////////sets 1 hour for breaktime each day after two subjects..
				    	if (mysql_num_rows($breaktime)==2)
				    		$start = date("H:i", strtotime($class['END_TIME']) + 60*60);

	            $timestart = date("h:i A", strtotime($start));
	            if (isset($lab))
	            	$timeend = date("h:i A", strtotime($timestart)+$lab*60*60/2);
	           	else
	            	$timeend = date("h:i A", strtotime($timestart)+$lec*60*60/2);
	            $opening = date("h:i A", strtotime("7:00"));
	            $closing = date("h:i A", strtotime("18:00"));
	            if ($timeend<$closing&&$timeend>$opening){
		            $seccheck = checkSection($day1,$timestart,$timeend);
		            $roomcheck = findRoom(1,$day1,$timestart,$timeend);
		            if (!$seccheck&&!$roomcheck){
			    		$_SESSION['day1'] = $day1;
						$_SESSION['start1'] = $timestart;
						$_SESSION['end1'] = $timeend;
		            }
		        }
	...
	for ($day2=$_SESSION['day1']+2; $day2 < 7; $day2++) { 
		for($j=7;$j<18;$j++){
	        foreach ($min as $v){
	            ...
	            $closing = date("h:i A", strtotime("18:00"));
	            if ($timeend<$closing&&$timeend>$opening){
		            $seccheck = checkSection($day2,$timestart,$timeend);
		            $roomcheck = findRoom(2,$day2,$timestart,$timeend);
		            if (!$seccheck&&!$roomcheck){
			    		$_SESSION['day2'] = $day2;
						$_SESSION['start2'] = $timestart;
						$_SESSION['end2'] = $timeend;
		            }
		        }
